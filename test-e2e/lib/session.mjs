 ...
 *
 * Adapted from the Magic Shop's harness; parameterised by user so a player client can join too.
 */

import { chromium } from "playwright";
import { BASE_URL, GM_USER, HEADED, MODULE_ID, WORLD_READY_TIMEOUT_MS } from "../config.mjs";
...
const KNOWN_NOISE = [
  ...
  {
    // Foundry's own: `ChatLog#postNotification` reaches into a chat-notifications element a
...
export class Session {
  ...
  static async open({
    ...
    const browser = await chromium.launch({
      ...
      args: [
        ...
        // Headroom: a headless renderer's default heap is small, and a world with Ember active
        // loads a great deal of script.
  ...
  async inWorld(file, exportName, arg) {
    // The closure runs in the page, where this file's imports do not exist: the module id travels
    // in as an argument.
    return this.page.evaluate(async ({ moduleId, file, exportName, arg }) => {
      const url = `/modules/${moduleId}/test-e2e/in-world/${file}`;
      const module = await import(url);
      if ( typeof module[exportName] !== "function" ) {
        throw new Error(`${file} has no exported function "${exportName}"`);
      }
      return module[exportName](arg);
    }, { moduleId: MODULE_ID, file, exportName, arg });
  }
