...
- **It knows boots from cloaks.** D&D 5e files every wondrous item under one type, so the loadout reads
...
- **It's the same equipment as the sheet.** Putting something on the loadout equips it; taking it off
  unequips it. AC, attunement and everything else that reads "equipped" updates as normal. Equip
  something from the inventory tab and it shows up on the loadout.
...
- **Two weapon sets, like Baldur's Gate 3.** Main hand and off hand sit on the left; the two ranged
  slots on the right are a ranged main hand and off hand. Each pair follows the two-handed rule on
  its own: a longbow in the first ranged slot blocks the second, but leaves your melee off hand
  free. Two hand crossbows fill both ranged slots.
...
- **The loadout wears Ember's look.** Ember's dark codex ground, parchment text and fonts, the same
...
- **It stays out of Ember's character builder.** Ember gives a new character its own fullscreen
  creation sheet. The loadout doesn't dock beside that, and the hotkey won't open it. Once Ember hands
  the finished character to the normal D&D 5e sheet, the Loadout tab and dock work as usual.
...
To send a homebrew item to a particular slot, set `flags["sogrom-simple-dnd5e-loadout"].slot` on
...
Found a bug or have an idea? [Open an issue](https://github.com/IainFielding/Simple-DnD-Loadout/issues).
